...
from django_deploy_kit.installer import Installer
...
class TestDryRun:
    ...
    @mock.patch("django_deploy_kit.installer.is_root", return_value=True)
    def test_dry_run_never_calls_subprocess(self, mock_root, config):
        """In dry-run mode, subprocess.run should never be called."""
        with mock.patch("django_deploy_kit.installer.run_system_command") as mock_cmd:
            with mock.patch("django_deploy_kit.installer.write_file_safe") as mock_write:
    ...
    def test_dry_run_returns_results(self, mock_root, config):
        ...
        with mock.patch("django_deploy_kit.installer.run_system_command"):
            with mock.patch("os.path.exists", return_value=False):
                with mock.patch("os.path.islink", return_value=False):
                    installer = Installer(config, dry_run=True)
                    results = installer.install()
...
class TestDefaultNginxRemoval:
    ...
    def test_warns_and_pauses(self, mock_sleep, mock_root, config):
        ...
        with mock.patch("django_deploy_kit.installer.write_file_safe"):
            with mock.patch("django_deploy_kit.installer.run_system_command"):
